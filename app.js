...
import * as queryAnswers from './query-answers.js';
import * as radioNav from './radio-nav.js';
...
$(document.body)
  ...
  .on('keydown', '.question input[type=radio]', function (e) {
    radioNav.handle(this, e);
  })
  .on('input', '.metadata-input', function () {
    state.metadata[this.dataset.metaField] = metadata.readInput(this);
    metadata.save(state.metadata);
    metadata.writeToQuery(state.metadata);
  })
...
function restoreHeader() {
  ...
  metadata.restoreInputs(state.metadata);
}
...
function restoreAnswersFromQuery() {
  const answers = queryAnswers.read(getQuestions());
  if (!answers) return;
...
function writeAnswersToQuery() {
  queryAnswers.write(state.answers, getQuestions());
}
...
window.pbq = {
  setLang(newLang) {
    ...
    state.lang = newLang;
    localStorage.setItem(keys.lang, newLang);
    document.documentElement.lang = newLang;
    document.querySelectorAll('[data-hx-frag]').forEach((el) => {
      el.setAttribute('hx-get', `./locales/${newLang}/${el.dataset.hxFrag}`);
      window.htmx?.process(el);
    });
    window.htmx?.trigger(document.body, 'langchange');
  },
  ...
  renderResults() {
    ...
    window.pbq.__lastRows = results.render({
